...
d = zeros(1, 1001);
i = 1;
while (x(1, i)^2 + x(2, i)^2)^0.5 >= 0.06
    x(:, i + 1) = A \ x(:, i);
    d(i) = (x(1, i)^2 + x(2, i)^2)^0.5;
    i = i + 1;
end
A9 = i - 1;
A10 = (x(1, i)^2 + x(2, i)^2)^0.5;
...
function output = R(theta)
    output = [cos(theta) 0 sin(theta); 0 1 0; -sin(theta) 0 cos(theta)];
end
